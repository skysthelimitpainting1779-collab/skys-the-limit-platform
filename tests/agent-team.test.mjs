...
function hostRuntimeState(result, name) {
  if (result.error?.code === "ENOENT") {
    assert.match(process.env.CI ?? "", /^(1|true)$/i, `${name}: HOST_RUNTIME_UNAVAILABLE is permitted only in CI`);
    return "HOST_RUNTIME_UNAVAILABLE";
  }
  assert.ifError(result.error);
  return "HOST_RUNTIME_AVAILABLE";
}
...
test("Codex execpolicy runtime rejects dangerous commands or is explicitly unavailable in CI", (t) => {
  ...
  const ruleSource = readFileSync(rules, "utf8");
  assert.match(ruleSource, /^# GENERATED by npm run host:compile$/m);
  assert.deepEqual([...ruleSource.matchAll(/prefix_rule\(\s*pattern = \[([^\]]+)\],\s*decision = "forbidden",/g)].map((match) => match[1]), [
    '"git","add","."',
    '"git","add","-A"',
    '"git","commit","-a"',
    '"git","reset","--hard"',
  ...
  const runtime = hostRuntimeState(result, "codex");
  if (runtime === "HOST_RUNTIME_AVAILABLE") {
    assert.equal(result.status, 0, result.stderr);
    assert.match(result.stdout, /forbidden/);
  } else {
    assert.equal(runtime, "HOST_RUNTIME_UNAVAILABLE");
    t.diagnostic("codex: HOST_RUNTIME_UNAVAILABLE; generated forbidden-rule artifact certified");
  }
});
...
test("host hooks are deterministic and Entire runtime is certified or explicitly unavailable in CI", (t) => {
  ...
  for (const event of ["session-start", "user-prompt-submit", "post-tool-use", "stop"]) {
    assert.match(JSON.stringify(codex), new RegExp(`entire hooks codex ${event}`));
  }
  ...
  for (const name of ["prepare-commit-msg", "commit-msg", "post-commit", "post-rewrite", "pre-push"]) {
    assert.match(readFileSync(join(root, ".husky", name), "utf8"), new RegExp(`entire hooks git ${name}`));
  }
  ...
  const runtime = hostRuntimeState(installed, "entire");
  if (runtime === "HOST_RUNTIME_AVAILABLE") {
    ...
    assert.equal(runtime, "HOST_RUNTIME_UNAVAILABLE");
    t.diagnostic("entire: HOST_RUNTIME_UNAVAILABLE; generated Codex and Git hook artifacts certified");
  }
});
